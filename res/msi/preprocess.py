...
g_arpsystemcomponent = {
    ...
    "Contact": {
        "msi": "ARPCONTACT",
        "v": "https://github.com/edbminfo/bmdesk",
    },
    "HelpLink": {
        "msi": "ARPHELPLINK",
        "v": "https://github.com/edbminfo/bmdesk/issues/",
    },
    "ReadMe": {
        "msi": "ARPREADME",
        "v": "https://github.com/edbminfo/bmdesk",
...
def make_parser():
    ...
    parser.add_argument(
        "--app-name", type=str, default="BMDesk", help="The app name."
    )
    parser.add_argument(
...
def replace_app_name_in_langs(app_name):
    ...
    for file_path in langs_dir.glob("*.wxl"):
        with open(file_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        for i, line in enumerate(lines):
            lines[i] = line.replace("BMDesk", app_name)
...
def update_license_file(app_name):
    ...
    license_content = license_content.replace("BMDesk", app_name)

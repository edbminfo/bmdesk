...
mod tests {
    ...
    #[test]
    fn update_download_file_accepts_bmdesk_github_asset_urls() {
        let file = get_download_file_from_url(
            "https://github.com/edbminfo/bmdesk/releases/download/1.4.0/bmdesk-1.4.0-x86_64.exe",
        )
        .expect("valid BMDesk GitHub release asset URL");

        assert_eq!(
            file.file_name().and_then(|name| name.to_str()),
            Some("bmdesk-1.4.0-x86_64.exe")
        );
    }
